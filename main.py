...
def get_security_indicator(score: int):
    if 0 <= score <= 25: return "🔴 (Sangat Rentan)"
    if 26 <= score <= 70: return "🟠 (Perlu Perbaikan)"
    if 71 <= score <= 85: return "🟢 (Aman)"
    return "🔵 (Sangat Aman)"
...
async def scan_website(request: SecurityScanRequest, x_api_key: str = Header(...)):
    ...
    data = perform_scan(request.target_url)
    score = data.get("score", 0)
    
    data["status_visual"] = get_security_indicator(score)
